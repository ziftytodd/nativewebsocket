...
import type { ConnectedState, DisconnectedState, MessageEvent, NativeWebsocketPlugin } from './definitions';
...
export class NativeWebsocketWeb extends WebPlugin implements NativeWebsocketPlugin {
  ...
  async connect(options: { url: string }): Promise<{ result: string }> {
    ...
    this.webSocket.onopen = () => {
      ...
      const ret: ConnectedState = { connected: true };
    ...
    this.webSocket.onmessage = async (msg) => {
      ...
      const ret: MessageEvent = { data: data, binary: msg.data instanceof ArrayBuffer };
    ...
    this.webSocket.onerror = (err) => {
      ...
      this.webSocket = null;
      const ret: DisconnectedState = { reason: 'PWA Unknown', error: 'PWA Unknown' };
